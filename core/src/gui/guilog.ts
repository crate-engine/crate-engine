...
export function guiLogPath(home: string): string {
  return join(tierPaths(home).root, "logs", "gui.log");
}

/** Open the log for a spawned child's stdout/stderr (the `crate open` side). */
export function openGuiLogFd(home: string): number {
  const p = guiLogPath(home);
  mkdirSync(dirname(p), { recursive: true });
  return openSync(p, "a");
}

export function guiLog(home: string, line: string): void {
  try {
    const p = guiLogPath(home);
    mkdirSync(dirname(p), { recursive: true });
 ...
 */
export function installGuiCrashLog(home: string, cleanup?: () => void): void {
  const die = (kind: string, detail: string, code: number) => {
