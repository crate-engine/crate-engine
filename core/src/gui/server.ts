...
export async function startGuiServer(
  ...
  const studioRunning = async (): Promise<Set<string>> => new Set((await import("./workspaces.js")).desiredRunning(home));
  ...
  const stopWorkspace = async (projectRoot: string) => {
    ...
    try {
      ...
      switch (route) {
        ...
        case "POST /api/workspaces/view": {
          ...
          if (!workspace?.exists || !workspace.rig) return json(res, 404, { error: "Project unavailable; locate it with Open Project or reconnect its drive." });
        ...
        case "POST /api/workspaces/open": {
          ...
          if (fresh && wasLive) return json(res, 409, { error: "Resume fresh is for a stopped workspace — this one is running. Stop it first." });
          if (fresh) {
          ...
          try {
            ...
            if (fresh) {
              ...
              const msg =
                `[RESUME_FRESH] The operator resumed this workspace FRESH — every seat starts a clean conversation. ` +
        ...
        case "GET /api/studio/state": {
          ...
          if (proj && viewer && first && studio.touchViewer(proj, viewer)) await studio.tick(await studioRunning());
          let probeOk = false;
          if (first && proj) {
        ...
        case "GET /api/fleet": {
          ...
          if (url.searchParams.get("fresh") === "1") await refreshConnected(); // menus + the quit note: current, not cached
          const { hostname, platform } = await import("node:os");
        ...
        case "POST /api/fleet/workspace": {
          ...
          if (!target) return json(res, 502, { error: `${host} is not connected right now — Computers ▸ Connect first` });
          const r = await runWorkspaceAction(target, action as import("./fleet.js").WorkspaceAction, path);
