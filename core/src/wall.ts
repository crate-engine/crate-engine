...
import { loadLoadout, loadoutPath, loadProjectDoors, type Seat } from "./manifest.js";
import { renderWallPlan, specFromLoadout, stateDoorsFor, type WallPlan } from "./sandbox.js";

/** Harnesses that may NEVER run unwalled (their approvals are bypassed). */
const WALL_REQUIRED: Record<string, string> = { claude: "claude-code", codex: "codex" };
...
let bwrapProbe: { bin: string; error?: string } | undefined;
function assertBwrapFunctional(bin: string): void {
  ...
  if (bwrapProbe.error) {
    throw new Error(
      `bubblewrap is installed but CANNOT create its sandbox on this machine (${bwrapProbe.error}). ` +
 ...
 */
export function brainRootFor(projectRoot: string): string | undefined {
 ...
 */
export function resolveHeadlessWall(
  projectRoot: string,
  seatArg: Seat | string,
  agentArg: string,
  opts: { platform?: NodeJS.Platform; home?: string; bwrapBin?: string } = {},
): (WallPlan & { sandbox: string }) | undefined {
  const seat = seatArg;
  const agent = normalizeAgent(agentArg);
  const required = agent in WALL_REQUIRED;
  const brainRoot = brainRootFor(projectRoot);
  const hasLoadout = brainRoot !== undefined && existsSync(loadoutPath(brainRoot, seat as Seat));

  if (!hasLoadout) {
    if (required) {
      throw new Error(
        `REFUSING to run the ${seat} seat headless: ${agent} is staffed with no loadout manifest, so no ` +
  ...
  const loadout = loadLoadout(brainRoot!, seat as Seat);
  if (loadout.policy.sandbox === "none") {
    if (required) {
      throw new Error(
        `REFUSING to run the ${seat} seat headless: ${agent} on a loadout that declares sandbox: none — ` +
  ...
  const projDoors = loadProjectDoors(projectRoot, seat as Seat);
  ...
  if (plan.skippedDoors.length > 0) {
    console.warn(
      `[wall] ${seat}: door(s) the ${plan.backend} backend cannot express, SKIPPED (writes there will be denied): ` +
        plan.skippedDoors.join("  "),
    );
  }
  if (plan.absentDoors.length > 0) {
    console.warn(
      `[wall] ${seat}: door(s) not present on disk, bound try-only (writes there are denied until the path exists — ` +
