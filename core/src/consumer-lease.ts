...
`;
const live = new WeakSet<ConsumerLease>();
export interface ConsumerLease { readonly key: string; release(): Promise<void> }
function keyFor(root: string, seat: string): string {
  if (!/^[a-z][a-z0-9_-]*$/.test(seat)) throw new Error("Invalid consumer seat");
  const dir = join(realpathSync(root), ".agents", "state", "turns", seat);
  mkdirSync(dir, { recursive: true });
  return join(realpathSync(dir), "consumer.lock");
}
export function assertConsumerLease(lease: ConsumerLease, root: string, seat: string): void {
  if (!live.has(lease) || lease.key !== keyFor(root, seat)) throw new Error("Consumer lease is not held for this seat");
}
export async function acquireConsumerLease(root: string, seat: string, waitMs = 0): Promise<ConsumerLease> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.once("close", () => {
      if (!acquired) reject(new Error(`Cannot own ${seat} queue: ${out.trim() === "BUSY" ? "another consumer is running" : stderr.trim() || "lock helper exited"}`));
    });
