...
const SEATS = ["orchestrator", "coder", "reviewer", "designer", "tester"] as const;

export function leftOffPath(projectRoot: string): string {
...
export function seatSummary(text: string): string[] {
  ...
  for (const l of lines) {
    ...
    if (take && l.trim()) picked.push(`  ${l.trim()}`);
  }
...
export function composeLeftOff(projectRoot: string, now = new Date()): string {
  ...
  if (branch) {
    ...
    for (const d of dirty.slice(0, 8)) out.push(`  - ${d.trim()}`);
    if (dirty.length > 8) out.push(`  - … ${dirty.length - 8} more`);
  }
  ...
  if (events.length) {
    ...
    for (const e of events.slice(-12)) out.push(`- ${e.length > 180 ? `${e.slice(0, 177)}…` : e}`);
  }
  return out.join("\n") + "\n";
}
...
export function writeLeftOff(projectRoot: string, now = new Date()): string | undefined {
  const dir = join(projectRoot, ".agents", "state", "checkpoints");
  if (!existsSync(join(projectRoot, ".agents", "state"))) return undefined;
  try {
