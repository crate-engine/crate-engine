...
export function buildInteractiveInvocation(
  ...
  switch (agent) {
    ...
    case "codex": {
      ...
      if (opts.walled) argv.push("--dangerously-bypass-approvals-and-sandbox");
    ...
    default:
      throw new Error(
        `the ${agent} seat has no interactive door yet — claude/codex/pi/agy only; ` +
...
export function paneResumeBanner(atIso: string, kind: PaneResumeKind = "engine-restart", reason?: string): Buffer {
...
export function scrubTerminalQueries(b: Buffer): Buffer {
  const s = b.toString("latin1"); // byte-preserving; the patterns are pure ASCII
  if (!TERMINAL_QUERY_RE.test(s)) return b;
...
export async function startSeatTty(opts: StartTtyOpts): Promise<StartTtyResult> {
  ...
  if (blended && resumingSession) {
    ...
    if (prior.length > 0) {
      const kind: PaneResumeKind = seenSeats.has(keyOf(projectRoot, seat)) ? "relaunch" : "engine-restart";
