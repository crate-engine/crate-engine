...
export type OpenArgs =
  ...
  | { kind: "remote"; host: string; printUrl: boolean }
  | { kind: "local"; project?: string; printUrl: boolean; stopOthers: boolean };

const VALUED = new Set(["--remote", "--project"]);
const KNOWN = new Set(["--remote", "--project", "--print-url", "--stop-others", "--force", "--cmux", "--help", "-h"]);

export function parseOpenArgs(rest: string[]): OpenArgs {
  if (rest.includes("--help") || rest.includes("-h")) return { kind: "help" };
  const positionals: string[] = [];
  for (let i = 0; i < rest.length; i++) {
    const a = rest[i]!;
    if (a.startsWith("-")) {
      if (!KNOWN.has(a)) return { kind: "error", message: `unknown option ${a}` };
      if (VALUED.has(a)) i++; // its value is not a positional
      continue;
    }
    positionals.push(a);
  }
  ...
  if (rIdx !== -1) {
    ...
    if (!host || host.startsWith("-")) return { kind: "error", message: "--remote <ssh-host> — the ssh host is missing" };
    if (positional)
      return {
        kind: "error",
