...
export function overlayMode(overlayFileText: string): OverlayMode {
...
export function composeFile(baseText: string | undefined, overlayText: string): string {
  if (overlayMode(overlayText) === "replace") return overlayText;
...
export function listOverlayEntries(overlayDir: string): OverlayEntry[] {
  const out: OverlayEntry[] = [];
  if (!existsSync(overlayDir)) return out;
  const walk = (dir: string) => {
    for (const name of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (name.isDirectory()) walk(abs);
        ...
        if (OVERLAY_INTERNAL.has(name.name)) continue;
        out.push({
...
// ── base-hash records (P4-6 compatibility pass) ─────────────────────────────
// overlay/.base-hashes.yaml maps entry relPath → sha256 of the brain base file
// the entry was (first) seen against; null = base absent at record time.

export type BaseHashes = Record<string, string | null>;

const baseHashFile = (overlayDir: string) => join(overlayDir, ".base-hashes.yaml");
...
export function readBaseHashes(overlayDir: string): BaseHashes {
  const f = baseHashFile(overlayDir);
  if (!existsSync(f)) return {};
  const raw = parse(readFileSync(f, "utf8"));
  return raw && typeof raw === "object" ? (raw as BaseHashes) : {};
}

export function writeBaseHashes(overlayDir: string, hashes: BaseHashes): void {
  mkdirSync(overlayDir, { recursive: true });
  writeFileSync(
    baseHashFile(overlayDir),
    `# managed by crate2 (P4-6): sha256 of the brain base file each overlay entry\n# was recorded against; updates flag entries whose base changed. Do not edit.\n${stringify(hashes)}`,
  );
}
 ...
 */
export function composedBrainRoot(brainRoot: string, overlayDir: string, outDir: string): string {
  const entries = listOverlayEntries(overlayDir);
  if (entries.length === 0) return brainRoot;
  ...
  if (hashesDirty) writeBaseHashes(overlayDir, hashes);

  return shadow;
}

/** Ensure shadowDir/name is a REAL directory mirroring brainDir (children symlinked). */
function expandDir(shadowDir: string, brainDir: string, name: string): string {
  const target = join(shadowDir, name);
  if (existsSync(target) && lstatSync(target).isSymbolicLink()) {
    const resolved = readlinkSync(target);
    unlinkSync(target);
    mkdirSync(target);
    for (const child of readdirSync(resolved)) linkInto(target, resolved, child);
  } else if (!existsSync(target)) {
    mkdirSync(target, { recursive: true });
    if (existsSync(brainDir)) for (const child of readdirSync(brainDir)) linkInto(target, brainDir, child);
  }
  return target;
}

function linkInto(dir: string, sourceDir: string, name: string): void {
  const link = join(dir, name);
  if (!existsSync(link) && !isLink(link)) symlinkSync(join(sourceDir, name), link);
}
...
export function overlayDirFor(home: string): string {
