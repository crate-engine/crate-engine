...
export function agentProblem(
  ...
  if (opts.deep && agent === "agy") {
    try {
      ...
      if (DEEP_RULES.agy.ready(out)) return undefined;
    } catch {
  ...
  if (opts.deep && (agent === "claude" || agent === "claude-code")) {
    try {
      ...
      if (DEEP_RULES.claude.ready(out)) return undefined;
    } catch {
      /* fall through to the honest problem */
    }
...
const DEEP_RULES = {
  agy: {
    bin: "agy",
    args: ["models"],
    ready: (out: string): boolean => /\S/.test(out.replace(/Fetching available models\.\.\.?/g, "")),
    problem: {
      agent: "agy",
      fix:
...
export function agentProblemAsync(
  agent: string,
  home: string,
  models: string[] = [],
  opts: { path?: string; deepTimeoutMs?: number } = {},
): Promise<AgentProblem | undefined> {
  const shallow = agentProblem(agent, home, models, { ...(opts.path !== undefined ? { path: opts.path } : {}) });
  if (shallow) return Promise.resolve(shallow);
  ...
  if (!bin) return Promise.resolve(rule.problem);
  const deepTimeout = opts.deepTimeoutMs ?? 30000;
  return new Promise((resolve) => {
    execFile(
      bin,
      [...rule.args],
      { encoding: "utf8", timeout: rule.bin === "claude" ? Math.min(15000, deepTimeout) : deepTimeout },
      (err, out) => resolve(!err && rule.ready(String(out)) ? undefined : rule.problem),
    );
  });
}
