...
// ── target resolution (the three v1-proven forms) ────────────────────────────
...
export function resolveTarget(
  ...
  opts: { cwd?: string; projectsRoot?: string; home?: string } = {},
): ResolvedTarget {
...
// ── the folder browser (run #4: attach needs real navigation, not a text box) ─
...
export function makeDir(rawParent: string | undefined, name: string, opts: { home?: string; roots?: string[] } = {}): DirListing {
  ...
  if (existsSync(target)) throw new AttachError(`"${clean}" already exists here — pick it in the list instead`);
  mkdirSync(target);
...
const LINK_PARTS = ["bin", "config", "adapters"] as const;
const DOC_SEEDS = ["AGENTS.md", "PROGRESS.md", "ISSUES.md"] as const;
 ...
 */
export function detectDevPort(projectRoot: string): number {
  try {
    ...
    const explicit =
      dev.match(/(?:--port|-p|-l)[ =](\d{2,5})/) ??
      dev.match(/http\.server (\d{2,5})/) ??
      dev.match(/\bPORT=(\d{2,5})/);
    if (explicit) return Number(explicit[1]);
    if (/\bastro\b/.test(dev)) return 4321;
...
function linkState(agentsDir: string, part: string, engineDir: string): WriteAction {
  ...
  if (!st.isSymbolicLink()) {
    throw new AttachError(
      `${link} exists and is NOT a symlink — migrate or remove it first ` +
        `(refusing to clobber real files; the engine links live here).`,
    );
  }
  const target = readlinkSync(link);
  if (target === join(engineDir, part) && existsSync(link)) return "keep";
...
export function planAttach(
  ...
  if (!existsSync(engineDir) || !existsSync(join(engineDir, "templates"))) {
    throw new AttachError(
      `no engine at ${engineDir} — run: crate2 setup (the brain clone must exist before attaching)`,
    );
  }
  const { projectRoot, project } = target;

  const mode: AttachPlan["mode"] = opts.create ? "create" : "attach";
  if (!target.exists && !opts.create) {
    throw new AttachError(
      `${projectRoot} does not exist. To attach, point at an existing folder; ` +
...
export function writeManagedGitignore(file: string): void {
  let outside = "";
  if (existsSync(file)) {
...
export function executeAttach(plan: AttachPlan, opts: { gitInit?: boolean; githubRepo?: boolean } = {}): AttachReport {
  ...
  if (plan.createsProjectDir) mkdirSync(projectRoot, { recursive: true });
  ...
  for (const w of plan.writes) {
    ...
    if (w.rel.startsWith(".agents/") && LINK_PARTS.includes(basename(abs) as (typeof LINK_PARTS)[number]) && !w.rel.includes("state")) {
      try {
  ...
  if (plan.mode === "create") {
    ...
    const docSeeds = changed.filter((rel) => (DOC_SEEDS as readonly string[]).includes(rel));
    if (docSeeds.length) {
      try {
        ...
        try {
          ...
          execFileSync(
            "git",
            ["-c", "user.email=crate@local", "-c", "user.name=crate2", "commit", "--quiet", "-m", msg, "--", ...docSeeds],
  ...
  let originMirror: string | undefined;
 ...
 */
export async function healDevUrl(
  projectRoot: string,
  opts: {
  ...
  if (!existsSync(confFile)) return undefined;
  ...
  while (await probeBusy(free)) free += 1; // busy-scan: first quiet port wins
