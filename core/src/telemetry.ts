...
import {
  ...
  renameSync, statSync, watch, writeFileSync, type FSWatcher,
} from "node:fs";
...
const SOURCES = ["events.log", "turns.log"] as const;
...
export function mirrorDir(projectRoot: string, home: string = homedir()): string {
  return join(home, ".crate", "logs", basename(projectRoot));
}

export function startTelemetryMirror(projectRoot: string, home: string = homedir()): TelemetryMirror {
  ...
  try {
    offsets = JSON.parse(readFileSync(offsetsFile, "utf8")) as Record<string, number>;
  } catch {
    offsets = {};
  }
  let queued = false;
  ...
  const header = (src: string) =>
    `# CRATE MIRROR — a COPY of ${join(stateDir, src)}\n` +
  ...
  const sync = () => {
    ...
    for (const src of SOURCES) {
      ...
      try {
        ...
        if (!existsSync(dst)) writeFileSync(dst, header(src));
        if (size < off) {
