...
import { findBwrap, renderBwrapArgs, writeProfile, type SandboxSpec, type SandboxPaths } from "../sandbox.js";
...
async function runProbe(wrapArgv: string[], probe: Probe): Promise<Result> {
  let denied = false;
  let detail = "";
  try {
...
function wrapFor(spec: SandboxSpec, paths: SandboxPaths, profileDir: string): string[] {
...
async function main(): Promise<void> {
  ...
  const specs: Array<{ label: string; spec: SandboxSpec }> = [
  ...
  const homeProbe =
    process.platform === "darwin" ? join(home, "Desktop", ".crate-sbtest-probe") : join(home, ".crate-sbtest-probe");
  const shared: Probe[] = [
  ...
  const perProfile: Record<string, Probe[]> = {
    readonly: [
  ...
  try {
    for (const { label, spec } of specs) {
      ...
      for (const probe of perProfile[label]!) {
        ...
        results.push(r);
        console.log(
          `[${label.padEnd(13)}] ${probe.name.padEnd(36)} ${probe.expect === "deny" ? "DENIED" : "ALLOWED"} ${r.pass ? "✓" : `✗ DEVIATION${r.detail ? ` (${r.detail})` : ""}`}`,
        );
      }
    }
  } finally {
  ...
  console.log(
    `SUMMARY: ${results.length} probes, ${failures.length} deviation(s).` +
