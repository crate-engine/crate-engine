...
export function devUrlFromRigConf(project: string): string | undefined {
...
export async function resolveBase(explicit: string | undefined, project: string, tool: string): Promise<string | undefined> {
  const base = (explicit ?? devUrlFromRigConf(project))?.replace(/\/+$/, "");
  if (!base) {
    console.log(
      `${tool}: no --base given and no DEV_URL in ${join(project, ".agents", "rig.conf")} — refusing to guess a port ` +
  ...
  if (/^https?:/.test(base)) {
    try {
      ...
      console.log(
        `${tool}: nothing is serving at ${base} — start the dev server (.agents/bin/dev-server up) or pass the right --base.`,
      );
      return undefined;
    }
  }
...
export function defaultCacheRoots(home: string): string[] {
...
export function chromiumFromCache(roots: string[]): string | undefined {
...
export function findChromium(): string {
  ...
  throw new Error(
    "no Playwright browser found — install one with: npx playwright-core install chromium-headless-shell",
  );
}
