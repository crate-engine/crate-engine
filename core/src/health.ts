...
export function normalizePaneText(raw: string | Buffer): string {
  ...
  return (
    s
      ...
      // what CSI never covered: ESC 7 / ESC 8 (DECSC/DECRC), ESC = / ESC > (keypad
      // modes), ESC ( B and friends (charset selection). Runs LAST so it can only
      // ever see what the three passes above left behind.
...
export function autoReviveEnabled(projectRoot: string): boolean {
...
export function makeAutoReviver(opts: {
  ...
  const episodes = new Map<Seat, { count: number; last: number; stoppedNoted: boolean }>();

  return {
    async tick(seats: SeatHealth[], workspace: string): Promise<ReviveNote[]> {
      const notes: ReviveNote[] = [];
      for (const s of seats) {
        if (s.liveness !== "dead") {
          if (s.liveness === "live") episodes.delete(s.seat); // healthy again → fresh episode next time
          continue;
        }
        ...
        if (e.count > 0 && now() - e.last < base * 2 ** e.count) continue; // backoff
        try {
