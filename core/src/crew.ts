...
export function buildCrewBundle(home: string, now: () => string = () => new Date().toISOString()): {
  bundle: CrewBundle;
  carried: string[];
  skipped: string[];
} {
...
export function writeCrewBundle(path: string, bundle: CrewBundle): void {
...
export function applyCrewBundle(home: string, raw: string): { written: string[] } {
  let bundle: CrewBundle;
  try {
    bundle = JSON.parse(raw) as CrewBundle;
  } catch {
  ...
  const allowed = new Set<string>(CREW_FILES);
  const written: string[] = [];
  for (const [rel, b64] of Object.entries(bundle.files)) {
    if (!allowed.has(rel)) throw new Error(`bundle names a path outside the crew set (${rel}) — refusing the whole file`);
    const abs = join(home, rel);
