...
export const CLAUDE_ALIASES = ["fable", "opus", "sonnet", "haiku"] as const;
...
let versionsCache: { at: number; value: HarnessVersions } | undefined;
/** Each installed harness's version (cached 10 minutes — it shells out). */
export function harnessVersions(pathOpt: { path?: string } = {}, now = Date.now()): HarnessVersions {
  if (versionsCache && now - versionsCache.at < 10 * 60_000) return versionsCache.value;
  const env = pathOpt.path ? { ...process.env, PATH: pathOpt.path } : process.env;
  const read = (bin: string): string | undefined => {
    try {
  ...
  const value: HarnessVersions = {};
  for (const bin of ["claude", "pi", "codex"] as const) {
    const v = read(bin);
    if (v) value[bin] = v;
  }
...
export function forgetHarnessVersions(): void {
...
export function modelForAlias(alias: ClaudeAlias, result: unknown): string | undefined {
  const usage = (result as { modelUsage?: Record<string, unknown> })?.modelUsage ?? {};
  return Object.keys(usage).find((k) => k.startsWith(`claude-${alias}`));
}

interface AliasCache {
  claudeVersion: string;
  aliases: Partial<Record<ClaudeAlias, string>>;
}

export function aliasCachePath(home: string): string {
...
export function claudeAliasModels(home: string, claudeVersion: string | undefined): Partial<Record<ClaudeAlias, string>> {
  if (!claudeVersion) return {};
  try {
    const c = JSON.parse(readFileSync(aliasCachePath(home), "utf8")) as AliasCache;
...
const defaultAsk = (pathOpt: { path?: string }): ClaudeAsk => async (alias) => {
  const { stdout } = await promisify(execFile)(
    "claude",
    ["-p", "--model", alias, "--output-format", "json", "Reply with only your exact model id."],
    {
      cwd: tmpdir(),
      timeout: 90_000,
      encoding: "utf8",
...
export function learnClaudeAliases(home: string, claudeVersion: string | undefined, ask: ClaudeAsk = defaultAsk({})): Promise<void> {
  if (!claudeVersion || !existsSync(tierPaths(home).root)) return Promise.resolve();
  const known = claudeAliasModels(home, claudeVersion);
  if (CLAUDE_ALIASES.every((a) => known[a])) return Promise.resolve();
  if (learning) return learning;
  learning = (async () => {
    const aliases: Partial<Record<ClaudeAlias, string>> = { ...known };
    for (const alias of CLAUDE_ALIASES) {
      if (aliases[alias]) continue;
      try {
        const id = modelForAlias(alias, await ask(alias));
        if (id) aliases[alias] = id;
      } catch {
        /* this alias stays unnamed (not on the plan, offline) — never fatal */
      }
    }
    writeFileSync(aliasCachePath(home), JSON.stringify({ claudeVersion, aliases } satisfies AliasCache, null, 2) + "\n");
  })().finally(() => {
    learning = undefined;
  });
  return learning;
...
export function claudeDisplay(alias: string, resolved: Partial<Record<ClaudeAlias, string>>, blurb?: string): string {
  const family = alias.charAt(0).toUpperCase() + alias.slice(1);
  const id = (resolved as Record<string, string | undefined>)[alias];
  if (id) return `Claude ${friendlyModel(id)} (Claude Code)${blurb ? ` — ${blurb}` : ""}`;
