...
switch (command) {
        ...
        try {
            ...
            if (r.before !== r.after) {
                ...
                if (ex(f)) {
                    ...
                    if (alive) {
                        console.log(`NOTE: a Crate Engine app is RUNNING — it keeps the old engine (${r.before.slice(0, 7)}) until relaunched. ` +
    ...
    case "team": {
        ...
        const staffing = SEATS.map((seat) => {
            const agentKey = `${RIG_PREFIX[seat]}_AGENT`;
            const agent = conf[agentKey] || "pi";
