...
function printStaffingTable(projectRoot) {
    ...
    for (const seat of SEATS) {
        ...
        if (flag)
            refusals.push([seat, d.agent.value]);
        console.log(`  ${seat.padEnd(13)} agent=${d.agent.value.padEnd(8)} [${d.agent.source}]`.padEnd(46) +
            `  model=${model} [${d.model.source}]${flag ? `  [WILL REFUSE — unwallable ${d.agent.value}]` : ""}`);
    }
    for (const [seat, agent] of refusals) {
        console.error(`  NOTE: ${seat} staffs ${agent} on a seat that cannot be walled (no loadout / sandbox: none) — ` +
...
switch (command) {
    ...
    case "update": {
        ...
        try {
            ...
            if (r.before === r.after)
                console.log(`crate2 update — already up to date (${r.after.slice(0, 7)})`);
            else
                console.log(`crate2 update — engine ${r.before.slice(0, 7)} → ${r.after.slice(0, 7)} (fast-forward)`);
            ...
            if (r.before !== r.after) {
                ...
                if (ex(f)) {
                    ...
                    if (alive) {
                        console.log(`NOTE: a Crate Engine app is RUNNING — it keeps the old engine (${r.before.slice(0, 7)}) until restarted. ` +
    ...
    case "crew": {
        ...
        if (sub === "export") {
            ...
            for (const c of carried)
                console.log(`  carried: ~/${c}`);
            for (const s of skipped)
                console.log(`  skipped (absent here): ~/${s}`);
            ...
            const file = rest[1];
            if (!file)
                fail("usage: crate crew import <bundle-file>");
            try {
    ...
    case "attach": {
        ...
        try {
            ...
            if (!target.exists && !flags.has("--create")) {
                if (assumeYes)
                    throw new AttachError(`${target.projectRoot} does not exist — pass --create to start a new project there.`);
            ...
            if (committed.length > 0) {
                ...
                for (const w of committed)
                    console.log(`  ${w.rel.padEnd(24)} ${w.note}`);
            }
            if (local.length > 0) {
                console.log(`\nLocal-only wiring (never pushed — auto-gitignored):`);
                for (const w of local)
                    console.log(`  ${w.rel.padEnd(24)} ${w.note}${w.action === "heal" ? "  [repair]" : ""}`);
            }
            if (keeps.length > 0) {
            ...
            if (plan.needsGit && plan.mode === "attach") {
                ...
                if (!gitInit)
                    console.log("  proceeding WITHOUT git — the team works, but there is no history/rollback.");
            }
            if (!assumeYes) {
            ...
            if (heavy.length > 0) {
                ...
                for (const d of heavy)
                    console.log(`  ${d.name.padEnd(20)} (${d.seat})  install: ${d.install}`);
                const yes = flags.has("--install-deps") || (!assumeYes && (await promptYesNo("Install them now (recommended)?")));
                if (yes) {
    ...
    case "open": {
        ...
        if (parsed.kind === "error")
            fail(`crate open: ${parsed.message}\n${OPEN_USAGE}`);
        ...
        if (parsed.kind === "remote") {
            ...
            try {
                ...
                if (!app) {
                    throw new Error(`no app url on ${host} (~/.crate/app-url) — is Crate installed there?\n` +
                ...
                console.log(`Crate Engine (on ${host}) is open — ${win.mode === "app" ? "the ⚡ app window" : "your browser"} is loading through the ssh tunnel.`);
                console.log(plan.teamUrl);
        ...
        if (project && !explicit)
            console.log(`crate open: opening the project here — ${project}`);
        if (explicit)
            console.log(`crate open: opening ${explicit}`);
            ...
            try {
                ...
                if (url) {
                    ...
                    try {
                        ...
                        const v = (await (await fetch(`${u0.origin}/api/version`, { headers: { "X-Crate-Token": tok }, signal: AbortSignal.timeout(8000) })).json());
                        const disk = diskEngineSha(home);
                        if (serverIsStale(v.loadedSha, disk)) {
                            console.log(`crate open: the running app server is on an OLD engine (${v.loadedSha ?? "pre-update"}; disk has ${disk}) — restarting it in place...`);
                            const r = (await (await fetch(`${u0.origin}/api/restart`, { method: "POST", headers: { "X-Crate-Token": tok }, signal: AbortSignal.timeout(60000) })).json());
                            if (r.ok === true && r.url) {
                                url = r.url.trim();
                                console.log(`crate open: fresh server is up on ${disk}.`);
                            }
                            else {
                                console.log(`crate open: the restart did not confirm — continuing with the RUNNING (old-engine) server. ` +
                ...
                if (rest.includes("--stop-others") || rest.includes("--force")) {
                    console.log("crate open: --stop-others is obsolete — workspaces no longer evict each other " +
                ...
                if (!url) {
                    ...
                    while (Date.now() - started < 60000 && !url) {
                        await new Promise((r) => setTimeout(r, 500));
                        if (ex(urlFile))
                            url = rf(urlFile, "utf8").trim();
                    }
                    if (!url)
                        throw new Error(`the app server did not come up (headless) — check ${guiLogPath(HOME)} and retry \`crate open\`.`);
                }
                ...
                if (project) {
                    try {
                        ...
                        if (!r.ok)
                            console.log(`crate open: ${j.error ?? "could not open the workspace"} — the window still opens; boot from the Team panel.`);
                    }
    ...
    case "stop": {
        ...
        if (sIdx !== -1) {
            ...
            if (!host || host.startsWith("--"))
                fail("crate stop --remote <ssh-host> — the ssh host is missing.");
            const { execFile } = await import("node:child_process");
            const { promisify } = await import("node:util");
            const sh = promisify(execFile);
            try {
                ...
                fail(`could not run the stop on ${host} over ssh — the server state there is UNKNOWN (NOT confirmed stopped): ` +
        ...
        if (scoped !== undefined) {
            ...
            if (!stoppedLive)
                setWorkspaceDesired(HOME, target, "parked");
            console.log(stoppedLive
        ...
        if (dead) {
            console.log(`crate stop: the app server is DOWN — confirmed (it stopped answering${pid ? `; pid was ${pid}` : ""}). ` +
                `Its team seats were stopped with it.`);
        }
        else if (!accepted) {
            fail(`crate stop: this server does not know /api/shutdown (it predates crate stop) and is STILL RUNNING — ` +
                `restart it onto the disk engine instead (crate open does this automatically), or kill it by hand.`);
        }
        else {
            fail(`crate stop: the server ACCEPTED the shutdown but is STILL ANSWERING after 10s${pid ? ` (pid ${pid})` : ""} — ` +
    ...
    case "recover": {
        if (process.env.CRATE_SEAT)
            fail("Recovery is an operator action; run it outside an agent seat");
        const seat = rest[0];
        if (!seat || !SEATS.includes(seat))
            fail("usage: crate recover <seat> --action resume|retry|complete --reason <inspection note> --project <rig>");
        const value = (flag) => { const i = rest.indexOf(flag); return i < 0 ? "" : rest[i + 1] ?? ""; };
    ...
    case "runner": {
        ...
        const seat = rest[0];
        if (!seat || !SEATS.includes(seat))
            fail(`usage: crate runner <${SEATS.join("|")}> [--project <path>] [--once]`);
        const pIdx = rest.indexOf("--project");
        const projectRoot = resolve(pIdx !== -1 ? (rest[pIdx + 1] ?? ".") : ".");
        const confFile = join(projectRoot, ".agents", "rig.conf");
        if (!existsSync(confFile))
            fail(`no rig.conf at ${confFile} — run crate install first`);
        ...
        // Refuse in plain words; BLEND_<PREFIX>=0 is the per-seat opt-out.
        {
            ...
            if (isBlended(conf, seat, agent)) {
                fail(`${seat} is BLENDED (the default for ${agent}) — the engine app (crate open) ` +
                    `delivers its mail into the live pane, and a second headless runner would double-consume the seat's ` +
                    `inbox. Set rig.conf BLEND_${RIG_PREFIX[seat]}=0 to opt this seat out and run it headless.`);
            }
        }
        ...
        console.log(`crate runner — ${seat} headless (${agent}${model ? `/${model}` : ""} ` +
    ...
    case "team": {
        ...
        const autoRefresh = ["1", "true", "yes", "on"].includes((conf.CONTEXT_AUTO_REFRESH || "").toLowerCase());
        if (autoRefresh)
            console.log("  context auto-refresh: ON (ceiling)");
        // team-defaults fix (FLAWS "crate team ignores ~/.crate/defaults.yaml"):
    ...
    case "print": {
        ...
        if (!SEATS.includes(seat))
            fail(`unknown seat "${rest[0]}" (expected one of: ${SEATS.join(", ")})`);
        // print never refuses (preflight off): it's the tool you debug a missing dep WITH.
        const { seats, brainRoot } = await planSeats(projectRoot, { preflight: false }).catch((e) => fail(e instanceof Error ? e.message : String(e)));
        ...
        console.log(`staffed:  ${plan.staffed.agent}${plan.staffed.model ? `/${plan.staffed.model}` : " (login picks)"}` +
