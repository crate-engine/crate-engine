...
export function buildHeadlessInvocation(agent, opts) {
...
export function parseUsage(agent, line) {
    let d;
    try {
        d = JSON.parse(line);
    }
    ...
    if (agent === "pi") {
        ...
        if (d.type === "message_end" && u)
            return { inputTokens: u.input ?? 0, outputTokens: u.output ?? 0 };
    }
    if (agent === "claude") {
        const u = d.usage;
        if (d.type === "result" && u)
    ...
    if (agent === "opencode") {
        ...
        const t = d.properties?.info?.tokens;
...
export function parseSessionId(agent, line) {
    let d;
    try {
        d = JSON.parse(line);
    }
    ...
    if (agent === "claude" && typeof d.session_id === "string")
        return d.session_id;
    if (agent === "codex" && d.type === "thread.started" && typeof d.thread_id === "string")
        return d.thread_id;
    if (agent === "opencode") {
        ...
        const sid = d.sessionID ??
            d.properties?.sessionID ??
            d.properties?.info?.sessionID;
 ...
 */
export function composeTurnPrompt(projectRoot, seat, mail, opts = {}) {
...
- Deliver results to a peer by appending to their inbox — ALWAYS sign it with --from ${seat}: python3 .agents/bin/agentctl.py deliver <role> --from ${seat} "<message>".${seat === "orchestrator"
