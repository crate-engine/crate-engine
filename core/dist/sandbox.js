 ...
 */
export function preseedAgyProjectTrust(home, projectRoot) {
    const cfg = join(home, ".gemini", "antigravity-cli", "settings.json");
    try {
        const data = JSON.parse(readFileSync(cfg, "utf8"));
        if (typeof data !== "object" || data === null)
            return false;
        const list = Array.isArray(data.trustedWorkspaces) ? data.trustedWorkspaces : [];
        if (list.some((w) => w === projectRoot))
            return false;
