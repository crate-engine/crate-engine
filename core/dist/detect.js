...
export function binaryFor(agent) {
    if (agent === "pi")
        return "pi";
    ...
    if (agent === "gemini")
        return "gemini";
    if (agent === "agy")
        return "agy"; // Antigravity CLI (the gemini wire's replacement)
    if (agent === "openclaw")
        return "openclaw";
    return undefined;
}
...
export function seatAuthProblem(agent, home, models = []) {
    if (agent === "pi") {
        let authJson;
        try {
    ...
    if (agent === "claude" || agent === "claude-code") {
        ...
        try {
            ...
            if (j.oauthAccount && j.hasCompletedOnboarding === true)
                return undefined;
        }
        catch {
            /* absent / unreadable */
    ...
    if (agent === "opencode") {
        ...
        if (existsSync(join(home, ".local", "share", "opencode", "auth.json")))
            return undefined;
        return {
            agent: "opencode",
    ...
    if (agent === "agy") {
        ...
        try {
            const j = JSON.parse(readFileSync(join(home, ".gemini", "antigravity-cli", "cache", "onboarding.json"), "utf8"));
...
export const DEEP_PROBED = ["claude", "claude-code", "agy"];
...
export function agentProblem(agent, home, models = [], opts = {}) {
