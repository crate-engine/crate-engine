...
function newestTurn(projectRoot, seat) {
    ...
    const meta = lines.map((l) => { try {
        return JSON.parse(l);
    }
    catch {
        return {};
    } }).find((d) => d.turnMeta);
    const startMs = (meta?.startedAt ? Date.parse(meta.startedAt) : undefined) || startMsFromName(name);
...
export function stateIsFresh(projectRoot, seat) {
    const t = newestTurn(projectRoot, seat);
    if (!t)
        return true; // no turns
    if (!t.complete)
        return false; // mid-turn — never refresh under a running turn
    const stateFile = join(projectRoot, ".agents", "state", `${seat}.md`);
    if (!existsSync(stateFile))
        return false; // work happened but no state written
...
export function refreshSeat(projectRoot, seat, opts = {}) {
    ...
    appendFileSync(join(turnsDir(projectRoot, seat), "turns.log"), `${new Date().toISOString()} | refreshed${opts.force ? " (forced)" : ""} | session dropped — next turn re-orients from state\n`);
    return { ok: true };
}
