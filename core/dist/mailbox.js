...
export function localIsoOffset(d = new Date()) {
...
let seq = 0;
function seatDir(inboxRoot, seat, sub) {
...
export function auditLog(inboxRoot, seat) {
...
export function complete(inboxRoot, seat, msgs) {
    const cur = seatDir(inboxRoot, seat, "cur");
    for (const m of msgs)
        renameSync(m.path, join(cur, m.name));
}
/**
 ...
 */
export function deadLetter(inboxRoot, seat, msg, reason) {
    const cur = seatDir(inboxRoot, seat, "cur");
