...
export function fileValue(file, key) {
    if (!existsSync(file))
        return undefined;
    let text;
    try {
...
export function confValue(projectRoot, key) {
    return (fileValue(join(projectRoot, ".agents", "rig.conf"), key) ??
...
function fromConf(projectRoot) {
    ...
    const previewPort = Number(fileValue(rig, "PREVIEW_DEV_PORT")) || Number(fileValue(dev, "PREVIEW_DEV_PORT")) || port;
    return { port, previewPort, origin: "conf-fallback" };
}
export function resolveDevPorts(projectRoot) {
    ...
    if (existsSync(resolver)) {
        try {
            ...
            const pick = (k) => Number(new RegExp(`^${k}=(\\d+)$`, "m").exec(out)?.[1] ?? 0);
            const port = pick("PORT");
            const previewPort = pick("PREVIEW_PORT");
            if (port)
                return { port, previewPort: previewPort || port, origin: "serve-resolve" };
        }
        catch {
...
//# sourceMappingURL=devport.js.map