...
export async function deliverToBlendedSeat(o) {
    ...
    for (const attempt of [1, 2]) {
        ...
        while (!o.signal?.aborted) {
            ...
            if (now() - deliveryT0 >= boundaryCeilingMs)
                break; // pathological pane — honest failure, never forever
            if (now() - t0 >= verifyTimeoutMs) {
                ...
                if (recent === undefined || recent < boundaryBusyBytes)
                    break;
            }
...
function flattenPane(replayText) {
...
function currentScreen(replayText) {
...
export function needsUpdateSkip(replayText) {
 ...
 */
export async function codexBootModals(readReplay, tty, opts = {}) {
    const sleep = opts.sleep ?? realSleep;
    const timeoutMs = opts.timeoutMs ?? 15_000;
    const pollMs = opts.pollMs ?? 250;
    const t0 = Date.now();
    ...
    const done = new Set();
    while (Date.now() - t0 < timeoutMs) {
        if (done.size === modals.length)
            return answered; // nothing left we know how to answer
