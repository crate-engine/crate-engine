...
export function verifyDelivered(jsonlText, marker, cli) {
    for (const line of jsonlText.split("\n")) {
        ...
        if (cli === "claude") {
            ...
            const msg = d.message;
            if (d.type === "user" && msg?.role === "user" && textParts(msg.content).includes(marker))
                return true;
        }
        else if (cli === "pi") {
            // probed: {"type":"message","message":{"role":"user","content":[{"type":"text","text":...}]}}
            const msg = d.message;
            if (d.type === "message" && msg?.role === "user" && textParts(msg.content).includes(marker))
                return true;
        }
        else {
            ...
            if (d.type === "event_msg" && payload?.type === "user_message" && typeof payload.message === "string" && payload.message.includes(marker))
                return true;
            if (d.type === "response_item" && payload?.role === "user" && textParts(payload.content).includes(marker))
                return true;
        }
    }
    return false;
