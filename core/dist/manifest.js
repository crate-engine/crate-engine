...
const ProjectDoorsSchema = z
    ...
    .strict();
export function projectDoorsPath(projectRoot) {
 ...
 */
export function loadProjectDoors(projectRoot, seat) {
    ...
    if (!parsed.success)
        throw new ManifestError(plainWords(file, parsed.error));
    const valid = new Set([...SEATS, "all"]);
    for (const k of Object.keys(parsed.data.doors)) {
        if (!valid.has(k)) {
            throw new ManifestError(`${file}: unknown seat "${k}" — seats are ${SEATS.join(", ")}, plus "all" for every seat`);
        }
    }
