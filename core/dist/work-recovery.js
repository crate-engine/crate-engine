...
export function saveWork(root, seat, record) {
    const path = file(root, seat), tmp = path + ".tmp-" + randomUUID();
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(tmp, JSON.stringify(record) + "\n", { mode: 0o600 });
    renameSync(tmp, path);
}
export function holdWork(root, seat, reason = recoveryMessage(seat)) {
    const work = readWork(root, seat);
    if (work)
        saveWork(root, seat, { ...work, heldReason: reason });
}
export function workHoldReason(root, seat) {
    try {
        return readWork(root, seat)?.heldReason;
    }
    catch {
        return "Work record is unreadable; inspection required before restarting";
    }
}
export function clearWork(root, seat) { rmSync(file(root, seat), { force: true }); }
export function finishRecordedMail(root, seat, record) {
    const inbox = join(root, ".agents", "state", "inbox");
    complete(inbox, seat, readNew(inbox, seat).filter(m => record.messages.includes(m.name)));
}
export function recoveryMessage(seat) {
...
export async function resolveWork(root, seat, action, reason) {
    if (!["resume", "retry", "complete"].includes(action) || !reason.trim())
        throw new Error("Recovery requires an action and a reason");
    const lease = await acquireConsumerLease(root, seat);
    try {
        const record = readWork(root, seat);
        if (!record)
            throw new Error("No interrupted work record for this seat");
        if (action === "resume" && (record.mode !== "blended" || !record.sessionId || !record.agent))
            throw new Error("Resume requires a known interactive session; inspect before choosing retry or complete");
        if (record.pid) {
            let alive = true;
            try {
            ...
            if (alive)
                throw new Error(`Previous work process ${record.pid} is still alive; stop and inspect it first`);
        }
        if (action === "resume") {
        ...
        if (action !== "retry")
            finishRecordedMail(root, seat, record);
        else {
            ...
            for (const name of record.messages) {
                ...
                if (existsSync(pending))
                    continue; // recovery itself may have restarted
                if (!existsSync(received))
                    throw new Error(`Recorded message ${name} is missing; preserve recovery evidence`);
                renameSync(received, pending);
            }
        }
        if (action === "resume")
            saveWork(root, seat, { ...record, phase: "received", resumeApproved: true, heldReason: undefined });
        else
            clearWork(root, seat);
    }
    finally {
