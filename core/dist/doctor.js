...
async function httpStatus(url, timeoutMs = 10000) {
...
export async function runDoctor(projectRoot) {
    ...
    results.push(dangling.length === 0
    ...
    if (dangling.length === 0) {
        try {
            ...
            results.push(existsSync(coreNm)
...
export function rigAuthProblems(projectRoot, home) {
    const agents = join(projectRoot, ".agents");
    const conf = existsSync(join(agents, "rig.conf"))
        ? parseRigConf(readFileSync(join(agents, "rig.conf"), "utf8"))
        : {};
...
export async function heavyDeps(projectRoot) {
    const brainRoot = deriveBrainRoot(projectRoot);
    const out = [];
    const seen = new Set();
    for (const seat of SEATS) {
        ...
        for (const dep of loadout.cli_deps) {
            if (!dep.heavy || !dep.install || seen.has(dep.install))
                continue;
...
export async function installHeavyDeps(deps) {
    const results = [];
    for (const d of deps) {
        try {
