...
export function studioPlan(projectRoot, engineBin) {
    const resolve = (kind) => {
        try {
...
export function demandPath(projectRoot) {
    return join(projectRoot, ".agents", "state", "studio-demand.json");
}
function designerLeaseUntil(projectRoot) {
    try {
        const d = JSON.parse(readFileSync(demandPath(projectRoot), "utf8"));
...
export class StudioServers {
    ...
    entries = new Map();
    plans = new Map();
    constructor(engineBin, log = () => undefined, now = Date.now) {
    ...
    plan(projectRoot) {
        if (!this.plans.has(projectRoot))
            this.plans.set(projectRoot, studioPlan(projectRoot, this.engineBin) ?? null);
    ...
    touchViewer(projectRoot, viewer) {
    ...
    users(projectRoot, e) {
        const t = this.now();
        for (const [k, at] of e.viewers)
            if (t - at > VIEWER_TTL_MS)
                e.viewers.delete(k);
    ...
    async tick(running) {
        ...
        for (const p of running)
            if (!this.entries.has(p) && designerLeaseUntil(p) > this.now())
                this.entry(p);
        for (const [p, e] of this.entries) {
            ...
            if (wanted)
                e.wantedAt = this.now();
            if (wanted && !alive && !e.starting)
                await this.start(p, e);
            else if (!wanted && alive && (!running.has(p) || this.now() - e.wantedAt > GRACE_MS))
                this.stop(p, e, running.has(p) ? "nobody has used it for 2 minutes" : "the workspace is not running");
            else if (!wanted && !alive && !running.has(p))
                this.entries.delete(p);
        }
    }
    async start(projectRoot, e) {
        e.starting = true;
        try {
            if (await portBusy(e.plan.port)) {
                // someone else holds the port (e.g. a hand-started server) — never fight it
                if (!e.blocked)
                    this.log(`studio preview: ${projectRoot} — port ${e.plan.port} is already in use; not starting the engine preview`);
                e.blocked = `port ${e.plan.port} is already in use by another server`;
                return;
            }
            e.blocked = undefined;
            const env = { ...process.env, CRATE_PROJECT: projectRoot, STATIC_SERVE_BIND: e.plan.bind };
    ...
    stopAll() {
        for (const [p, e] of this.entries)
            if (e.child)
                this.stop(p, e, "the engine is shutting down");
    }
...
//# sourceMappingURL=studioserve.js.map