...
import { blendEligible, isBlended, sessionUsage, sessionWorkState } from "../blend.js";
...
function plainTool(name, arg) {
    const a = arg.toLowerCase();
    if (name === "read")
        return "Reading the project files";
    if (name === "write")
        return "Writing up its notes";
    if (name === "edit")
        return "Editing the code";
    if (name === "bash" || name === "$") {
        if (/\bnpm test\b|node test|jest|vitest|pytest|\btest\.js/.test(a))
            return "Running the tests";
        if (/git commit/.test(a))
            return "Saving the changes";
        if (/git push/.test(a))
            return "Publishing the work";
        if (/git (diff|show|log|status)/.test(a))
            return "Reviewing what changed";
        if (/nm-gate|quality|lint|typecheck|build/.test(a))
            return "Running the quality checks";
        if (/agentctl\s+deliver/.test(a))
            return "Messaging a teammate";
        if (/agentctl\s+(emit|state|check)/.test(a))
            return "Updating its status";
        if (/\b(find|ls|cat|grep|rg|head|tail)\b/.test(a))
...
export function liveTail(events, n = 8) {
...
export function narratedDigest(events) {
    const tools = [];
    let lastText;
    for (const e of events) {
        if (e.kind === "text") {
        ...
        if (e.kind === "tool" || e.kind === "result" || e.kind === "stderr") {
            ...
            const line = firstClause(e.kind === "tool" ? (e.plain ?? e.narrated) : e.narrated);
            if (line && tools[tools.length - 1] !== line)
                tools.push(line);
        }
    }
    ...
    if (lastText) {
        ...
        if (concl && out[out.length - 1] !== concl)
            out.push(concl);
    }
    return out;
}
...
export function narrateLine(raw) {
    let d;
    try {
        d = JSON.parse(raw);
    }
    ...
    if (type === "tool_execution_end")
        return { narrated: "· done", raw, kind: "other" };
    if (type === "message_update") {
        const ev = (d.assistantMessageEvent ?? {});
        if (String(ev.type ?? "").includes("thinking"))
            return { narrated: "thinking…", raw, kind: "think" };
    ...
    if (d.stderr)
        return { narrated: `⚠ ${String(d.stderr).trim().slice(0, 100)}`, raw, kind: "stderr" };
    // claude stream-json
    if (type === "system")
        return { narrated: `· session ${String(d.subtype ?? "event")}`, raw, kind: "system" };
    if (type === "assistant" || type === "text" || type === "message_end" || type === "turn_end") {
        const text = extractText(d);
        if (text)
            return { narrated: text.slice(0, 140), raw, kind: "text" };
    }
    if (type === "result")
        return { narrated: `✓ ${String(d.result ?? "done").slice(0, 140)}`, raw, kind: "result" };
    // codex exec
    if (type === "item.completed") {
        const item = d.item;
        if (item?.type === "agent_message" && item.text)
            return { narrated: item.text.slice(0, 140), raw, kind: "text" };
        if (item?.type?.includes("command"))
            return { narrated: `$ ${item.command ?? item.type}`.slice(0, 140), raw, kind: "tool" };
        return { narrated: `· ${item?.type ?? "item"}`, raw, kind: "other" };
    }
    if (type === "turn.completed")
        return { narrated: "· turn complete", raw, kind: "other" };
    if (type === "thread.started" || type === "turn.started")
        return { narrated: `· ${type}`, raw, kind: "system" };
...
export function streamEvent(raw) {
    let d;
    try {
        d = JSON.parse(raw);
    }
    ...
    if (type === "message_update") {
        ...
        if (ev.type === "text_delta" && ev.delta)
            return { k: "td", t: ev.delta };
        if (ev.type === "text_end" && ev.content)
            return { k: "text", t: ev.content };
    ...
    if (type === "tool_execution_end") {
        const isError = Boolean(d.result?.isError);
        ...
        if (lines.length >= FOLD_MIN_LINES)
            return { k: "fold", t: `… ${lines.length.toLocaleString()} lines of output` };
    ...
    if (d.stderr)
        return { k: "stderr", t: `⚠ ${String(d.stderr).trim().slice(0, 200)}` };
    ...
    if (type === "item.completed") {
        const item = d.item;
        if (item?.type === "agent_message" && item.text)
            return { k: "text", t: item.text };
        if (item?.type?.includes("command"))
            return { k: "tool", t: `$ ${item.command ?? item.type}`, p: plainTool("bash", item.command ?? "") };
        return undefined;
    }
...
export function turnStartMs(name) {
    return startedMsFromName(name);
}
function extractText(d) {
    const msg = d.message;
    if (msg?.content)
        return msg.content.filter((c) => c.type === "text").map((c) => c.text).join(" ").trim() || undefined;
    if (typeof d.text === "string")
        return d.text;
    return undefined;
}
/** Reconstruct epoch ms from a turn file's ISO name (colons became dashes). */
function startedMsFromName(name) {
...
function readTurn(path) {
    ...
    const startedAtMs = (meta?.startedAt ? Date.parse(meta.startedAt) : undefined) ?? startedMsFromName(name);
    const lt = runningTokens(parsed);
    return {
        file: name,
        startedAt: meta?.startedAt ?? name.replace(".jsonl", ""),
