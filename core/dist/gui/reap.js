...
export function canonProject(p) {
    try {
...
export function tagFromPsLine(line) {
...
export function selfAndAncestors() {
...
export async function terminate(pids, graceMs = 4000) {
    ...
    const t0 = Date.now();
    while (Date.now() - t0 < graceMs && targets.some(alive))
...
export function taggedFor(project, all = listTagged()) {
 ...
 */
export async function teardownWorkspace(project, opts = {}) {
    let devServer;
 ...
 */
export async function sweepStopped(stopped, graceMs) {
    if (!stopped.size)
        return [];
    const keep = selfAndAncestors();
    const byProject = new Map();
    for (const t of listTagged()) {
        if (keep.has(t.pid) || t.selfHosted)
            continue;
