...
export function qaGreen(qa) {
    const head = qa.split(/\r?\nLog:/)[0] ?? qa; // current status only — the log is history
    if (/partial|\bFAIL(ED)?\b|BUGS_FOUND|BLOCKER|CHANGES_NEEDED/i.test(head))
        return false;
    const status = head.match(/^status:\s*(.+)$/im)?.[1]?.trim().toLowerCase();
    if (status)
        return /^(pass|passed|verified|green|ok)\b/.test(status);
...
export function pendingGates(projectRoot) {
    ...
    if (approved.length > 0) {
        for (const [task] of approved)
            cards.push({ task, branch: task, deploysTo: dep, ...v });
    }
