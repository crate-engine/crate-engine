...
export function remotesFile(home) {
    return join(tierPaths(home).root, "remotes.json");
}
export function listRemotes(home) {
...
export function validRemoteHost(host) {
...
export function probeArgv(host) {
...
export function installArgv(host) {
...
export function bootArgv(host) {
    return [...SSH_OPTS, host, '"$HOME/.local/bin/crate" open'];
}
export function appUrlArgv(host) {
    return [...SSH_OPTS, host, "cat ~/.crate/app-url"];
}
export function defaultRemoteExec() {
    return {
...
const jobs = new Map();
export function remoteJob(host) {
    return jobs.get(host);
}
/** Tests only: forget finished jobs so runs stay hermetic. */
export function clearRemoteJobs() {
    jobs.clear();
}
function stderrTail(e) {
    const err = e;
...
async function runConnect(job, home, exec) {
    job.phase = "booting";
    ...
    if (!app)
        throw new Error(`no app url on ${job.host} (~/.crate/app-url) — the engine did not come up`);
    ...
    if (!up) {
        throw new Error(`the tunnel to ${job.host}:${app.port} did not come up — is local port ${app.port} taken? (stop whatever holds it and retry)`);
    }
...
function startJob(home, host, exec, first) {
    const live = jobs.get(host);
    if (live && live.phase !== "failed" && live.phase !== "connected")
        return live; // one job per host
    const job = { host, phase: "probing", note: `reaching ${host} over ssh…`, log: [], startedAt: new Date().toISOString() };
    jobs.set(host, job);
    void (async () => {
        try {
            await first(job);
...
export async function probeRemote(host, exec = defaultRemoteExec()) {
    try {
        ...
        return {
            reachable: false,
            engine: false,
            note: `ssh could not reach ${host} with your keys — ` +
...
export function startConnect(home, host, exec = defaultRemoteExec()) {
...
export function startInstall(home, host, exec = defaultRemoteExec()) {
