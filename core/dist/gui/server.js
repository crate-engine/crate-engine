...
import { executeAttach, listDirs, makeDir, planAttach, resolveTarget } from "../attach.js";
...
const MODELS = [
    {
        ...
        verifiedFor: ["orchestrator", "reviewer", "designer", "tester"],
    },
...
export function engineVersionFast(home) {
    const { engineDir } = tierPaths(home);
...
export function restartArgv(state, urlFile) {
...
async function cachedDeepProblem(agent, home, pathOpt) {
    if (!DEEP_PROBED.includes(agent))
        return undefined;
    const now = Date.now();
    ...
    if (cached && (cached.problem === undefined || now - cached.at <= 30_000))
 ...
 * CE-173 lesson). */
const sshReach = new Map();
function sshReachable(host) {
    const c = sshReach.get(host);
    if (c && Date.now() - c.at < 60_000)
        return Promise.resolve(c.ok);
...
export function warmDeepVerdicts(home, pathOpt) {
    return Promise.all([...new Set(DEEP_PROBED)].map((a) => cachedDeepProblem(a, home, pathOpt).catch(() => undefined))).then(() => undefined);
}
const CLAUDE_BLURBS = { fable: "Anthropic's top tier", haiku: "fastest, lightest" };
...
function learnsAliases(home, detectPath) {
...
async function staffingCatalog(state, detectPath, project) {
    ...
    if (learnsAliases(state.home, detectPath))
        void learnClaudeAliases(state.home, claudeVersion).catch(() => undefined);
    ...
    const curated = await Promise.all(MODELS.map(async (m) => {
...
function writeDefaults(state, seats) {
    const { defaultsFile } = tierPaths(state.home);
    const previous = existsSync(defaultsFile) ? readFileSync(defaultsFile, "utf8") : undefined;
    const text = `# ~/.crate/defaults.yaml — your global staffing defaults (written by the Crate Engine app).\n` +
    ...
    try {
        ...
        if (previous !== undefined)
            writeFileSync(defaultsFile, previous);
...
const studioProbes = new Map(); // keyed by target — two workspaces' slots must not thrash one cache slot
function probePreviewTarget(target) {
...
export async function startGuiServer(opts = {}) {
    ...
    if (requested !== undefined && !existsSync(join(requested, ".agents", "bin"))) {
        console.error(`crate2 gui: --project ${requested} has no .agents/bin (not an attached project — typo?) — ` +
    ...
    const ensureProxyFor = async (project) => {
        ...
        const proxy = createServer((req, res) => {
            ...
            const preq = httpRequest({ hostname: t.hostname, port: t.port || 80, path: req.url, method: req.method, headers: { ...req.headers, host: t.host } }, (pres) => {
    ...
    const spawnerFor = () => opts.seatSpawner ?? defaultSeatSpawner(state.cliPath, state.home);
    ...
    const ensureMirror = (projectRoot) => {
        if (!mirrors.has(projectRoot))
            mirrors.set(projectRoot, startTelemetryMirror(projectRoot, home));
    };
    const stopMirror = (projectRoot) => {
    ...
    const stopWorkspaceProcesses = async (projectRoot) => {
    ...
    const healIfNeeded = (projectRoot) => {
        const { engineDir } = tierPaths(home);
        try {
            if (!existsSync(join(engineDir, "templates")) || projectState(projectRoot, engineDir) !== "heal")
                return;
    ...
    const server = createServer(async (req, res) => {
        try {
            ...
            if (url.pathname.startsWith("/fonts/")) {
                ...
                if (!/^[a-z0-9-]+\.woff2$/.test(name) || !existsSync(file))
                    return json(res, 404, { error: "no such font" });
            ...
            switch (route) {
                ...
                case "GET /api/team": {
                    ...
                    const proj = url.searchParams.get("project") ?? state.project;
                    if (!proj)
                        return json(res, 200, { project: "(none)", seats: [] });
                ...
                case "GET /api/team/status": {
                    ...
                    const proj = url.searchParams.get("project") ?? state.project;
                    if (!proj)
                        return json(res, 200, { booted: false, seats: [] });
                ...
                case "GET /api/preview": {
                    ...
                    const proj = url.searchParams.get("project") ?? state.project;
                    if (!proj)
                        return json(res, 200, { previews: [], proxyPort: null });
                    return json(res, 200, {
                ...
                case "POST /api/preview/point": {
                    ...
                    if (!/^http:\/\//.test(target))
                        return json(res, 400, { error: "only http:// targets proxy — https targets open direct" });
                ...
                case "POST /api/context/refresh": {
                    ...
                    const seat = String(body.seat ?? "");
                    if (!seat)
                        return json(res, 400, { error: "seat required" });
                ...
                case "GET /api/stream": {
                    ...
                    const proj = url.searchParams.get("project") ?? state.project;
                    if (!proj)
                        return json(res, 400, { error: "no project attached yet" });
                ...
                case "POST /api/staffing/seat": {
                    ...
                    if (!agent)
                        return json(res, 400, { error: "agent required" });
                    const confFile = join(proj, ".agents", "rig.conf");
                    if (!existsSync(confFile))
                        return json(res, 400, { error: "no rig.conf — attach the project first" });
                ...
                case "GET /api/tty/stream-all": {
                    ...
                    const unsubs = ttys.map((tty) => {
                        ...
                        return tty.subscribe((ev) => {
                            try {
                                if (ev.data)
                                    res.write(`data: ${JSON.stringify({ seat: tty.seat, d: ev.data.toString("base64") })}\n\n`);
                                if (ev.exit)
                                    res.write(`data: ${JSON.stringify({ seat: tty.seat, exit: ev.exit.code })}\n\n`);
                            }
                            catch {
                    ...
                    req.on("close", () => {
                        ...
                        if (viewClient)
                            for (const t of liveTtyList(proj))
                                t.dropSizeProposal(viewClient);
                    });
                    return;
                }
                ...
                case "POST /api/tty/resize": {
                    ...
                    if (cols > 0 && rows > 0)
                        tty.resize(cols, rows, body.client ? String(body.client) : undefined);
                    return json(res, 200, { ok: true });
                }
                ...
                case "POST /api/restart": {
                    ...
                    const handoff = handoffStop();
                    if (handoff.stopped > 0)
                        guiLog(state.home, `restart: stopped ${handoff.stopped} seats for handoff`);
                    ...
                    while (Date.now() - t0 < 30000 && !newUrl) {
                        ...
                        if (existsSync(urlFile))
                            newUrl = readFileSync(urlFile, "utf8").trim();
                    }
                    if (!newUrl)
                        return json(res, 500, { error: "the fresh app server did not come up — reopen with: crate open" });
                ...
                case "POST /api/crew/terminal": {
                    ...
                    const body = await readBody(req);
                    const crew = { pi: "pi", claude: "claude", codex: "codex" };
                ...
                case "POST /api/fs/native-pick": {
                    ...
                    const script = mode === "choose-folder"
                    ...
                    try {
                        ...
                        if (/canceled|cancelled|-128/i.test(msg))
                ...
                case "GET /api/ssh-hosts": {
                    ...
                    const hosts = await Promise.all(computers.map(async (c) => ({
                        name: c.name,
                        aliases: c.aliases,
                ...
                case "GET /api/projects": {
                    ...
                    if (computer === "" || computer === "local") {
                        ...
                        const recents = listWorkspaces(state.home).map((w) => ({
                            path: w.path,
                            ...(typeof w.focusedAt === "number" ? { focusedAt: w.focusedAt } : {}),
                        }));
                        return json(res, 200, {
                    ...
                    const remote = (await defaultFleetExec()
                        ...
                        .catch(() => null));
                    if (!remote)
                        return json(res, 502, { error: `${computer} stopped answering — try again` });
                ...
                case "POST /api/projects/open": {
                    ...
                    if (!r || r.error || !r.project)
                        return json(res, 502, { error: r?.error ?? `${computer} gave no answer` });
                    return json(res, 200, {
                        project: r.project,
                ...
                case "POST /api/attach/plan": {
                    const body = await readBody(req);
                    const target = resolveTarget(body.target, { home: state.home });
                    const plan = planAttach(target, tierPaths(state.home).engineDir, {
                ...
                case "POST /api/deps/install-one": {
                    ...
                    if (!dep)
                        return json(res, 200, { name: body.name, ok: true, detail: "already installed" });
                ...
                case "GET /api/health": {
                    ...
                    const proj = url.searchParams.get("project") ?? state.project;
                    if (!proj)
                        return json(res, 400, { error: "no project attached yet — attach one first" });
                    ...
                    const seats = st.seats.map((s) => {
                        if (s.alive) {
                            ...
                            if (problem)
                                return { seat: s.seat, liveness: problem.liveness, detail: problem.detail };
        ...
        try {
            ...
            if (swept.length) {
                ...
                for (const r of swept)
                    guiLog(home, `sweep: ${r.project} is not running — closed ${r.closed} leftover process(es)`);
            }
        }
        catch {
    ...
    server.on("close", () => {
        ...
        for (const m of mirrors.values())
            m.stop();
        mirrors.clear();
        for (const p of previewProxies.values())
            p.close();
    ...
    if (learnsAliases(home, opts.detectPath))
        void learnClaudeAliases(home, harnessVersions().claude).catch(() => undefined);
    ...
    for (const w of (await import("./workspaces.js")).listWorkspaces(home)) {
        if (w.rig)
            await ensureProxyFor(w.path);
    }
    if (state.project)
        await ensureProxyFor(state.project);
