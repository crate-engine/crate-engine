...
let deepClaudeVerdict;
function cachedDeepClaudeProblem(agent, home, pathOpt) {
    if (agent !== "claude")
        return undefined;
    const now = Date.now();
...
function staffingCatalog(state, detectPath) {
    // Provenance needs a brain: prefer the current project's, else the user tier's clone.
    const brain = state.project !== undefined ? deriveBrainRoot(state.project) : tierPaths(state.home).engineDir;
