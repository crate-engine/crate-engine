...
export async function runTurn(opts) {
    ...
    try {
        ...
        if (result.recoveryRequired)
            holdWork(opts.projectRoot, opts.seat, result.error);
        return result;
    }
    finally {
        if (!opts.consumerLease)
            await lease.release();
    }
}
async function runOwnedTurn(opts) {
    ...
    const work = { version: 1, mode: "headless", phase: "prepared",
        id: randomUUID(), messages: mail.map(m => m.name), at: startedAt.toISOString(), sessionId, logPath };
    saveWork(projectRoot, seat, work);
    let result;
    try {
        result = await execTurn(inv, projectRoot, logPath, agent, opts.turnTimeoutMs ?? DEFAULT_TURN_TIMEOUT_MS, { ...seatEnv(projectRoot, seat), ...(walled ? { CRATE_WALLED: "1" } : {}) }, pid => saveWork(projectRoot, seat, { ...work, pid }));
    }
    finally {
        try {
...
export function execTurn(inv, cwd, logPath, agent, timeoutMs, env, onSpawn) {
...
export async function runnerLoop(opts) {
    ...
    assertConsumerLease(lease, opts.projectRoot, opts.seat);
    try {
        await ownedRunnerLoop({ ...opts, consumerLease: lease });
    }
    finally {
        if (!opts.consumerLease)
            await lease.release();
...
async function ownedRunnerLoop(opts) {
    ...
    try {
        while (!opts.signal?.aborted) {
            ...
            if (r.recoveryRequired) {
                ...
                while (!opts.signal?.aborted && getPpid() === ppid0)
                    await idleWait();
                return;
            }
