 ...
 */
export function buildInvocation(loadout, staffed, paths) {
    const brain = (rel) => resolve(paths.brainRoot, rel);
    const argv = ["pi"];
    if (staffed.model !== "")
        argv.push("--model", staffed.model);
    ...
    for (const f of loadout.append_system)
        argv.push("--append-system-prompt", brain(f));
    argv.push("--no-skills");
    for (const s of loadout.skills)
        argv.push("--skill", brain(s));
    argv.push("--no-prompt-templates");
    for (const t of loadout.prompt_templates)
        argv.push("--prompt-template", brain(t));
...
export function toShellCommand(inv) {
    const q = (s) => (/^[A-Za-z0-9_@%+=:,./-]+$/.test(s) ? s : `'${s.replaceAll("'", `'\\''`)}'`);
