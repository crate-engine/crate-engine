...
export function tierPaths(home) {
    if (!home)
        throw new UserTierError("cannot locate the user tier without HOME");
...
export function seedDefaultsIfAbsent(home, opts = {}) {
    const t = tierPaths(home);
    if (existsSync(t.defaultsFile))
        return false;
    ...
    if (!claudeReady && piReady) {
        seed = seed.replace("  coder: { agent: claude, model: opus }", "  coder: { agent: pi, model: openai-codex/gpt-5.5 } # claude not detected on this machine at first start");
    }
 ...
 */
export function setupTier(home, opts) {
    const t = tierPaths(home);
    const actions = [];
    mkdirSync(t.root, { recursive: true });
    if (existsSync(t.overlayDir))
        actions.push("kept    overlay/");
    else {
        mkdirSync(t.overlayDir, { recursive: true });
        actions.push("created overlay/");
    }
    if (existsSync(t.defaultsFile))
        actions.push("kept    defaults.yaml (yours — not touched)");
    else {
    ...
    if (existsSync(t.engineDir)) {
        if (!existsSync(join(t.engineDir, ".git"))) {
            throw new UserTierError(`${t.engineDir} exists but is not a git clone — move it aside and re-run setup ` +
        ...
        try {
            ...
            throw new UserTierError(`could not clone the brain from ${opts.engineSource}: ${e instanceof Error ? e.message : e}`);
        }
...
export function writeLastProject(home, projectRoot) {
    const t = tierPaths(home);
    if (!existsSync(t.root))
        return; // no tier, nothing to persist into
    writeFileSync(join(t.root, "last-project"), `${projectRoot}\n`);
}
export function readLastProject(home) {
...
export function appUrlPath(home) {
 ...
 */
export function updateEngine(home) {
    const t = tierPaths(home);
    if (!existsSync(join(t.engineDir, ".git"))) {
    ...
    for (const e of entries) {
        if (!(e.relPath in hashes))
            hashes[e.relPath] = hashFile(join(t.engineDir, e.relPath));
    }
    ...
    try {
        ...
        throw new UserTierError(`the engine clone at ${t.engineDir} did not fast-forward — it has local edits. ` +
...
//# sourceMappingURL=usertier.js.map