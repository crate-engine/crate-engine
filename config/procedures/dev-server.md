...
(e.g. docket), pass the path: `.agents/bin/dev-server <cmd> /mnt/data/projects/<repo>`.
...
## Config (layered: rig.conf → dev.conf → env → defaults)
- `DEV_PORT` (else parsed from `DEV_URL`), `DEV_BIND` (`0.0.0.0` — serves localhost AND
  LAN, survives an IP change), `DEV_HEAP_MB` (node `--max-old-space-size`), `DEV_MEMMAX`
  (systemd cap). A rig reads its `rig.conf`; a non-rig repo gets a small `.agents/dev.conf`.
